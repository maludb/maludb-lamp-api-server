 ...
 *   POST                          Create a skill. Body: {name, description?, version?,
 *                                 visibility?, packaging_kind?, enabled?}
 *
 ...
 * {system_prompt,markdown,mcp_tool,plugin}. The skill body/markdown is not exposed
 * by this facade view (see docs/db-requirements.md).
 */
    ...
    case 'POST': {
        ...
        $created = db_one(
            "INSERT INTO maludb_skill (" . implode(', ', $cols) . ")
             VALUES (" . implode(', ', $ph) . ")
             RETURNING skill_id AS id, skill_name AS name, description, version,
                       visibility, packaging_kind, enabled, created_at",
            $params
        );
